...
SELECT o.onum,c.cname,s.sname FROM orders o,customer c ,salespeople s WHERE o.cnum=c.cnum AND o.snum=s.snum ORDER BY o.onum DESC ;  --q2
...
SELECT city,MAX(rating) AS rating FROM customer GROUP BY city;  --q7

SELECT MIN(amt),cnum FROM orders GROUP BY cnum; --q8

SELECT * FROM customer WHERE cname LIKE 'G%' ORDER BY cname LIMIT 1; --q9
...
SELECT MAX(rating)FROM customer (SELECT rating FROM customer WHERE cnum=
(SELECT COUNT(cnum),cnum FROM orders GROUP BY cnum ORDER BY COUNT(cnum) DESC LIMIT 1)); --q13 (doubt)
...
SELECT sp.sname,sp.snum FROM salespeople sp,customer c WHERE sp.snum=c.snum  GROUP BY snum HAVING COUNT(DISTINCT cnum)>1; --q17

SELECT s.snum,s.sname,COUNT(o.onum) AS COUNT FROM salespeople s  LEFT JOIN orders o ON s.snum=o.snum
GROUP BY o.snum ORDER BY COUNT DESC ; --q18
...
SELECT snum FROM customer GROUP BY snum HAVING COUNT(DISTINCT cnum)>1; --q23

SELECT c.cname,c.city,sp.sname FROM customer c, salespeople sp WHERE c.city=sp.city AND c.snum=sp.snum; --q24

 SELECT  MAX(o.amt),sp.sname FROM orders o,salespeople sp 
 WHERE o.snum=sp.snum AND sname IN('serres','rikklin') GROUP BY sname; --q25
...
SELECT sname FROM salespeople WHERE city IN('barcelona','london'); --q26
 ...
 SELECT  DISTINCT(sp.sname),o.amt FROM orders o,salespeople sp WHERE o.snum=sp.snum AND 
 amt>(SELECT AVG(amt) FROM orders) ; --q28
 
 SELECT sname,comm FROM salespeople WHERE city='london'; --q29
  
  
SELECT COMM FROM salespeople INNER JOIN customer ON salespeople.`SNUM`=customer.`SNUM` AND
 customer.`CITY`='London' --q30
